    ...
    <div id="print-area" className="print-sheet bg-white text-[10px] leading-tight text-gray-800 max-w-[1000px] mx-auto">
      ...
      <div className="flex items-center justify-between border-b-2 border-[#1a3a5c] pb-2 mb-3">
        ...
        <div className="text-center">
          ...
          <div className="text-[9px] text-gray-600">{ac.model}</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          ...
          <table className="w-full border-collapse border border-gray-400 text-[9px]">
            <thead>
              <tr className="bg-[#1a3a5c] text-white">
                ...
                <th className="text-right px-1.5 py-1 font-semibold w-14">Brazo</th>
                ...
                return (
                  <tr key={s.id} className="border-b border-gray-300">
                    ...
                    <td className="px-1.5 py-0.5 text-right font-mono">
                      {w ? (s.is_fuel ? `${val} gal = ${w.toFixed(1)}` : w) : ''}
                    </td>
          ...
          {/* 3. Limites de rendimiento */}
          <div className="border border-gray-400 rounded p-2">
            <div className="text-[9px] font-bold text-[#1a3a5c] uppercase mb-1">Limites de Rendimiento</div>
            <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[9px]">
              <span className="text-gray-500">Max Takeoff Weight:</span>
              <span className="font-mono">{ac.max_gross.toLocaleString()} lb</span>
              <span className="text-gray-500">Max Landing Weight:</span>
              <span className="font-mono">{ac.max_landing.toLocaleString()} lb</span>
            </div>
          </div>
          ...
          <div className="border border-gray-400 rounded p-2">
            ...
            <div className="grid grid-cols-4 gap-x-2 gap-y-1 text-[9px]">
              {[
                ['DEP',     state.identification.dep],
                ['DEST',    state.identification.dest],
                ['DATE',    state.identification.date || state.flightData.date],
                ['REG',     ac.reg],
                ['TYPE',    ac.model],
                ['PIC',     state.identification.pic],
            ...
            <div className="grid grid-cols-4 gap-x-2 mt-1 text-[9px]">
              ...
              <div><span className="text-gray-500 text-[8px]">TOG: </span><span className="font-mono font-semibold">{cg.toFixed(2)}</span></div>
              <div><span className="text-gray-500 text-[8px]">LCG: </span><span className="font-mono font-semibold">{(wb.ldgCG || 0).toFixed(2)}</span></div>
            </div>
          </div>

          {/* 5. Plan de navegación compacto */}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-[9px]">
            <div className="border border-gray-300 rounded px-1.5 py-0.5">
              <span className="text-gray-500 text-[8px]">DEP ATIS: </span>
              <span className="font-semibold">{state.depAtis || '—'}</span>
            </div>
            <div className="border border-gray-300 rounded px-1.5 py-0.5">
              <span className="text-gray-500 text-[8px]">ARR ATIS: </span>
              <span className="font-semibold">{state.arrAtis || '—'}</span>
            </div>
          </div>
