...
export default function WBTable() {
  ...
  }, [state.wbInputs, state.ac, state.fuelBurn])
          ...
          {ac.stations.map(s => {
            ...
            return (
              <tr key={s.id} className="border-b border-gray-200">
                <td className="px-2 py-1.5 text-xs font-medium text-gray-600">
                  {s.label}
                  {s.is_fuel && ac.fuel_burn_note && (
                    <span className="block text-[9px] text-gray-400 mt-0.5">{ac.fuel_burn_note}</span>
                  )}
                </td>
